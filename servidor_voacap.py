...
def _extraer_url_prediction(html, response_url=None):
    """
    Busca la URL dinámica de prediction.txt.

    Primero revisa la URL final de la respuesta HTTP (puede existir
    un redirect). Luego busca el enlace dentro del HTML.
    """
    candidatos = []

    if response_url:
        candidatos.append(response_url)
    ...
    for contenido in candidatos:
        ...
        # Algunas respuestas HTML pueden traer barras escapadas.
...
def extraer_resultados(html, session, response_url=None):
    """
    ...
    url_prediction = _extraer_url_prediction(html, response_url)

    if not url_prediction:
        raise RuntimeError(
            "VOACAP no devolvió el enlace dinámico a prediction.txt. "
            "URL final de la respuesta: " + str(response_url)
        )
    ...
    print("URL prediction.txt:", url_prediction)
    print("Longitud prediction.txt:", len(prediction_txt))
...
def voacap():
    ...
    try:
        ...
        resultados = extraer_resultados(html, session, response_url)
...
if __name__ == "__main__":
    ...
    print("")
    print("Modo: SSB")
    print("Fuente FOT/MUF: prediction.txt REAL de VOACAP")
