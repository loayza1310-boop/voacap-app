...
def _numero_voacap(valor):
    """Extrae un número de un campo de VOACAP sin alterar el orden."""
    if valor is None:
        return None
    ...
    # Acepta enteros y decimales, pero no une números separados.
...
def _frecuencia_voacap(valor):
    """Devuelve una frecuencia VOACAP limpia."""
    if valor is None:
        return None
...
def extraer_resultados(html):
    """
    Extrae FREQ1/FREQ2/FREQ3 y FOT/MUF del bloque Best FREQ de
    una respuesta Method 30 de VOACAP.

    IMPORTANTE:
    En la respuesta real de VOACAP que estamos usando, las columnas
    pueden contener marcadores (?, *, --) y no siempre mantienen una
    cantidad fija de campos numéricos. Por eso NO se utilizan índices
    fijos como partes[12] y partes[13].

    La versión anterior que funcionaba ya demostraba que los TRES
    ÚLTIMOS valores numéricos de cada fila corresponden a:
        FOT, MUF, HPF

    El error original era exigir >=15 números. En determinadas horas
    VOACAP devuelve menos números porque algunos campos intermedios no
    son numéricos, aunque FOT/MUF sí están presentes.
    """
    ...
    for linea in texto.splitlines():
        ...
        # -------------------------------------------------
        # La fila debe comenzar con UTC 1..24
        # -------------------------------------------------
        if len(partes) < 2:
            continue
        ...
        # -------------------------------------------------
        # FRECUENCIAS
        # Cada UTC aparece tres veces: FREQ1/FREQ2/FREQ3.
        # -------------------------------------------------
        frecuencia = _frecuencia_voacap(partes[1])
        ...
        for valor in partes[1:]:
            numero = _numero_voacap(valor)
            if numero is not None:
                numeros.append(numero)

        # No exigimos 15 números.
        # Lo importante es que existan los tres valores finales.
        if len(numeros) >= 3 and utc not in parametros:
            parametros[utc] = {
                "fot": numeros[-3],
                "muf": numeros[-2]
            }
