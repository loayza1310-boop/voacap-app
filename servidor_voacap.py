...
VOACAP_URL = "https://www.voacap.com/hf/"



def extraer_url_prediction(html):
    patrones = [
        r'https?://(?:www\.)?voacap\.com/hf/predictions/[^"\'<>\s]+/prediction\.txt',
        r'/hf/predictions/[^"\'<>\s]+/prediction\.txt',
    ]
    for patron in patrones:
        m = re.search(patron, html or "", re.IGNORECASE)
        if m:
...
def extraer_fot_muf_prediction(txt):
    """Extrae únicamente GMT, FOT y MUF del prediction.txt de VOACAP."""
    resultados = []
    tabla = False
...
def consultar_voacap(datos):
    ...
    url_prediction = extraer_url_prediction(respuesta.text) or extraer_url_prediction(respuesta.url)
    if not url_prediction:
        raise RuntimeError("No se encontró la URL dinámica de prediction.txt en VOACAP.")
    pred_response = session.get(
        url_prediction,
        headers={"User-Agent": "Mozilla/5.0", "Referer": "https://www.voacap.com/hf/"},
        timeout=60
    )
    pred_response.raise_for_status()
    resultados = extraer_fot_muf_prediction(pred_response.text)
    if not resultados:
        raise RuntimeError("No se pudo extraer FOT/MUF de prediction.txt.")
